...
import { RULES, searchPlayers, type Axis, type Catalog, type Difficulty, type GridState, type Position } from '../../lib/football-grid/engine';
...
export default function FootballGridGame() {
  ...
  async function refresh(mode = difficulty, showRound = false) {
    ...
    try {
      ...
      setSeconds(Math.ceil(reply.remaining/1000)); setError(false);
  ...
  useEffect(()=>{
    ...
    Promise.all([rpc('easy'),rpc('hard')]).then(async replies=>{
      ...
      if(generation.current!==turn)return;
      setStates({easy:replies[0].value,hard:replies[1].value});deadline.current=performance.now()+reply.remaining;
      setSeconds(Math.ceil(reply.remaining/1000));setChoosing(!reply.value.round);setError(false);
  ...
  useEffect(()=>{
    ...
    const tick=setInterval(()=>{
      if(active){
        const left=Math.max(0,Math.ceil((deadline.current-performance.now())/1000));setSeconds(left);
  ...
  async function act(action:string, mode=difficulty, extra:Record<string,unknown>={}) {
    ...
    try {
      ...
      setSeconds(Math.ceil(reply.remaining/1000));setError(false);setChoosing(false);
  ...
  const achievements:[boolean,string][]=[[easyWins>0,t.achievementEasy],[hardWins>0,t.achievementHard],[easyWins>0&&hardWins>0,t.achievementBoth],[easyWins+hardWins>=10,t.achievementTen]];
  ...
  return <div className="football-grid">
    ...
    {user===undefined || (user&&!state&&!error) ? <p className="fg-panel" role="status">{t.loading}</p> : !user ? <section className="fg-panel fg-login"><span className="fg-kicker">FOOTBALL GRID · 4 × 4</span><h2>{t.login}</h2><p>{supabase?t.loginText:t.unavailable}</p><Link className="fg-primary" href={`/${locale}/usuario`}>{t.login} ↗</Link></section> : choosing || !round ? <section className="fg-panel fg-intro"><span className="fg-kicker">16 / 16 · FOOTBALL GRID</span><h2>{t.choose}</h2><div className="fg-modes">{(['easy','hard'] as const).map((mode,i)=>{
      const s=states[mode],inProgress=s?.round?.status==='active',limited=(s?.used??0)>=3&&!inProgress;
      return <article className="fg-mode" key={mode}><div className="fg-mode-number" aria-hidden="true">0{i+1}<span>4 × 4</span></div><h3>{t[mode]}</h3><p>{mode==='easy'?t.easyText:t.hardText}</p><div className="fg-mode-facts"><span>{RULES[mode].seconds} {t.seconds}</span><strong>+{RULES[mode].reward} {t.points}</strong></div><p className="fg-fine">{t.penalty}: −{RULES[mode].surrender} · {t.daily}: {s?.used??0}/3</p><button className="fg-primary" disabled={busy||limited||!s||error} onClick={()=>void act('start',mode)}>{inProgress?t.resume:limited?t.limit:t.start} <span aria-hidden="true">↗</span></button></article>;
      ...
      <div className="fg-scoreboard"><div className={active&&seconds<=10?'fg-time is-urgent':'fg-time'}><span>{t.time}</span><strong role="timer" aria-label={t.time} aria-live="off">{active?`${Math.floor(seconds/60)}:${String(seconds%60).padStart(2,'0')}`:'0:00'}</strong></div><div><span>{t.filled}</span><strong>{solved}<small> / 16</small></strong></div><div><span>{t.daily}</span><strong>{state?.used??0}<small> / 3</small></strong></div><div><span>{t.total}</span><strong>{state?.stats.points??0}</strong></div></div>
      {!active&&<div className={`fg-summary ${round.status==='won'?'is-win':''}`} role="status"><span>{t.summary}</span><h3>{round.status==='won'?t.complete:round.status==='timeout'?t.expired:t.surrendered}</h3><strong>{round.score>0?'+':''}{round.score} {t.points}</strong><button className="fg-primary" disabled={busy||(state?.used??0)>=3} onClick={()=>void act('start')}>{(state?.used??0)>=3?t.limit:t.again}</button></div>}
    ...
    <section className="fg-rules"><h2>{t.rules}</h2><div className="fg-rule-scores"><div><strong>{t.easy}</strong><span>120 s · +100 / 0 / −20</span></div><div><strong>{t.hard}</strong><span>90 s · +200 / 0 / −50</span></div></div><p>{t.scoring}</p><details><summary>{t.rules}</summary><p>{t.rulesText}</p><p>{t.quota}</p><p>{t.editRule}</p></details><p className="fg-fine">{t.reset}</p><details><summary>{catalog.players.length} {t.catalog}</summary><p>{t.data}</p><p>{new Intl.DateTimeFormat(locale,{dateStyle:'medium'}).format(new Date(catalog.exportedAt))} · {catalog.version}</p></details></section>
