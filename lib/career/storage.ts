...
export function parseCareer(raw:string|null):CareerState|null{if(!raw)return null;try{const x=JSON.parse(raw) as Partial<Envelope>;return x.schemaVersion===2&&valid(x.value)?hydrateCareer(x.value):null}catch{return null}}
