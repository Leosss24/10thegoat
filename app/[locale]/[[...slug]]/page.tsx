...
export default async function LocalizedPage({ params }: Props) {
  ...
  if (path === "") return <main><section className="hero container"><div className="hero-brand-banner-wrap"><img className="hero-brand-banner" src="/brand/10thegoat-home-banner-fade.png" alt={d.home.alt}/></div><div className="eyebrow">{d.home.eyebrow}</div><h1>{d.home.title}</h1><p>{d.home.intro}</p><div className="actions"><Link className="btn btn-primary" href={localizedPath(locale, "/juegos")}>{d.home.play}</Link><Link className="btn" href={localizedPath(locale, "/beta")}>{d.home.beta}</Link></div></section><section className="section container"><div className="section-heading-row"><div><span className="eyebrow">{d.home.version}</span><h2>{d.home.games}</h2></div><Link className="text-link" href={localizedPath(locale, "/juegos")}>{d.home.all}</Link></div><div className="grid">{games.map((game) => <GameCard key={game.slug} game={game} locale={locale}/>)}</div></section></main>;
