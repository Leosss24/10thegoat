...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    icons: { icon: [{ url: "/brand/10thegoat-shield-32x32.png", sizes: "32x32", type: "image/png" }, { url: "/brand/10thegoat-shield-64x64.png", sizes: "64x64", type: "image/png" }], apple: [{ url: "/brand/10thegoat-shield-256x256.png", sizes: "256x256", type: "image/png" }] },
  };
}
...
export default async function LocaleLayout({ children, params }: { children: React.ReactNode; params: Promise<{ locale: string }> }) {
  ...
  return <html lang={locale}><body><I18nProvider locale={locale} dictionary={d}>
    <header className="site-header"><div className="container nav">
      <Link className="brand" href={localizedPath(locale)} aria-label={`10theGOAT · ${d.nav.home}`}><img className="brand-logo" src="/brand/10thegoat-shield-64x64.png" alt=""/><span><strong>10</strong>the<strong>GOAT</strong></span><em className="beta-pill">BETA</em></Link>
    ...
    <footer className="footer"><div className="container footer-layout"><div className="footer-brand"><img src="/brand/10thegoat-shield-48x48.png" alt=""/><span><strong>10</strong>the<strong>GOAT</strong> · Beta</span></div><nav className="footer-links" aria-label={d.nav.legal}><Link href={localizedPath(locale, "/privacidad")}>{d.nav.privacy}</Link><Link href={localizedPath(locale, "/cookies")}>{d.nav.cookies}</Link><Link href={localizedPath(locale, "/aviso-legal")}>{d.nav.notice}</Link><Link href={localizedPath(locale, "/beta")}>{d.nav.feedback}</Link></nav></div></footer>
